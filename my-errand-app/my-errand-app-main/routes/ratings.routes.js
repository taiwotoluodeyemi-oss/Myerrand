...
router.post('/:errand_id', verifyToken, async (req, res) => {
  try {
    ...
    try {
      await pool.execute(
        `INSERT INTO ratings (errand_id, rater_id, rated_id, rating, review, communication_rating, punctuality_rating, quality_rating)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [errandId, raterId, ratedId, ratingNum, review || null, clamp(communicationRating), clamp(punctualityRating), clamp(qualityRating)]
      );
    } catch (err) {
