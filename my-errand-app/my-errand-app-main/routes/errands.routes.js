...
router.post('/create', verifyToken, requireClient, async (req, res) => {
  try {
    ...
    if (payAmount <= 0) {
      return jsonResponse(res, 400, false, null, 'amount must be greater than 0');
    }
    const [result] = await pool.execute(
      // payment_status starts as NULL (unpaid), not the string 'pending' — the
      // frontend's "Pay & Start" button only renders when payment_status is
      // falsy, and a non-empty string like 'pending' is always truthy in JS.
      `INSERT INTO errands (
         client_id, title, description, pickup_address, delivery_address,
         amount, budget_amount, estimated_hours, weight_kg, urgency, category,
         status, payment_status
       ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NULL)`,
      [clientId, title, description, pickup_address, delivery_address,
       payAmount, payAmount, estimated_hours || 1, weight_kg || 0, urgency || 'medium', category || null]
    );
